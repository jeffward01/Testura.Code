...
public class PropertyGeneratorTests
{
    ...
    {
        Assert.AreEqual(
            "publicstaticintMyProperty{get;set;}",
            PropertyGenerator.Create(
                    new AutoProperty(
                        "MyProperty",
                        typeof(int),
    ...
    public void Create_WhenCreatingAutoPropertyWithOnlyGet_ShouldHaveNoSet()
    {
        Assert.AreEqual(
            "intMyProperty{get;}",
            PropertyGenerator.Create(new AutoProperty("MyProperty", typeof(int), PropertyTypes.Get))
                .ToString());
    }
    ...
    {
        Assert.AreEqual(
            "[Test]intMyProperty{get{return1;}}",
            PropertyGenerator.Create(
                    new BodyProperty(
                        "MyProperty",
                        typeof(int),
    ...
    {
        Assert.AreEqual(
            "publicvirtualintMyProperty{get{return1;}}",
            PropertyGenerator.Create(
                    new BodyProperty(
                        "MyProperty",
                        typeof(int),
    ...
    {
        Assert.AreEqual(
            "intMyProperty{protectedget;set;}",
            PropertyGenerator.Create(
                    new AutoProperty(
                        "MyProperty",
                        typeof(int),
                        PropertyTypes.GetAndSet,
                        getModifiers: new List<Modifiers>
                        {
                            Modifiers.Protected
                        }))
                .ToString());
    ...
    {
        Assert.AreEqual(
            "intMyProperty{get;privateinternalset;}",
            PropertyGenerator.Create(
                    new AutoProperty(
                        "MyProperty",
                        typeof(int),
                        PropertyTypes.GetAndSet,
                        setModifiers: new List<Modifiers>
                        {
                            Modifiers.Private,
                            Modifiers.Internal
                        }))
