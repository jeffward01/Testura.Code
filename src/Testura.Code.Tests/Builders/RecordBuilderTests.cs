...
#pragma warning restore SA1201
    ...
    {
        Assert.IsTrue(
            _recordBuilder.WithAttributes(new Attribute("MyAttribute"))
                .Build()
                .ToString()
    ...
    {
        Assert.IsTrue(
            _recordBuilder.ThatInheritFrom(typeof(int))
                .Build()
                .ToString()
                .Contains("TestRecord:int"));
    }
    ...
    {
        Assert.IsTrue(
            _recordBuilder.WithModifiers(Modifiers.Public, Modifiers.Abstract)
                .Build()
                .ToString()
                .Contains("publicabstractrecordTestRecord"));
    }
    ...
    {
        _recordBuilder.WithPrimaryConstructor(new Parameter("par1", typeof(int)))
            .WithFields(new Field("hello", typeof(string)));

        Assert.IsTrue(
            _recordBuilder.Build()
                .ToString()
                .Contains("TestRecord(intpar1){"));
    }
    ...
    {
        Assert.IsTrue(
            _recordBuilder.WithUsings("some.namespace")
                .Build()
                .ToString()
