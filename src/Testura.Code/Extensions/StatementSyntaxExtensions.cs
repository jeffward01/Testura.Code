...
public static class StatementSyntaxExtensions
{
    ...
    public static StatementSyntax WithComment(
        this StatementSyntax statementSyntax, string comment,
        CommentPosition commentPosition = CommentPosition.Above)
    {
        switch (commentPosition)
        {
            case CommentPosition.Above:
                return statementSyntax.WithLeadingTrivia(
                    SyntaxFactory.TriviaList(SyntaxFactory.Comment($"//{comment}\n")));
            case CommentPosition.Right:
                return statementSyntax.WithTrailingTrivia(
                    SyntaxFactory.TriviaList(SyntaxFactory.Comment($" //{comment}\n")));
            default:
                throw new ArgumentOutOfRangeException(
                    nameof(commentPosition),
                    commentPosition,
                    null);
        }
    }
