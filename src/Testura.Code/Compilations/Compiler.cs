...
public class Compiler : ICompiler
{
    ...
    /// <returns>The result from the compilation.</returns>
    public async Task<CompileResult> CompileFilesAsync(
        string outputPath, params string[] pathsToCsFiles)
    {
        if (pathsToCsFiles.Length == 0)
        {
            throw new ArgumentException(
                "Value cannot be an empty collection.",
                nameof(pathsToCsFiles));
        }

        var source = new string[pathsToCsFiles.Length];
        for (var n = 0; n < pathsToCsFiles.Length; n++)
        {
    ...
    private IEnumerable<MetadataReference> ConvertReferenceToMetaDataReferfence()
    {
        ...
        metaData.Add(
            MetadataReference.CreateFromFile(Path.Combine(_runtimeDirectory, "mscorlib.dll")));
        metaData.Add(
            MetadataReference.CreateFromFile(Path.Combine(_runtimeDirectory, "System.dll")));
        metaData.Add(
            MetadataReference.CreateFromFile(Path.Combine(_runtimeDirectory, "System.Core.dll")));
