...
using Common.Arguments.ArgumentTypes;
...
public static class NunitAssertGenerator
{
    ...
    public static ExpressionStatementSyntax AreEqual(
        IArgument expected, IArgument actual, string message = null)
    {
    ...
    public static ExpressionStatementSyntax AreNotEqual(
        IArgument expected, IArgument actual, string message = null)
    {
    ...
    public static ExpressionStatementSyntax AreNotSame(
        IArgument expected, IArgument actual, string message = null)
    {
    ...
    public static ExpressionStatementSyntax AreSame(
        IArgument expected, IArgument actual, string message)
    {
    ...
    public static ExpressionStatementSyntax Contains(
        IArgument expectedContain, IArgument actual, string message = null)
    {
        ...
        {
            new InvocationArgument(
                Statement.Expression.Invoke(
                        actual.GetArgumentSyntax()
                            .ToString(),
                        "Contains",
                        new List<IArgument> { expectedContain })
                    .AsExpression()),
            new ValueArgument(message)
        };

        return Statement.Expression.Invoke("Assert", "IsTrue", arguments)
            .AsStatement();
    }
    ...
    /// <param name="message">Message if test fails.</param>
    /// <returns>The declared expression statement syntax.</returns>
    public static ExpressionStatementSyntax Throws(
        VariableReference variableReference, Type exception, string message = null)
    {
        ...
        if (variableReference is not MethodReference)
        {
            ...
            if (member is not MethodReference)
            {
                throw new ArgumentException(
                    $"{variableReference} or last member in chain must be a method");
            }
        }

        var arguments = new List<IArgument>
        {
            new ParenthesizedLambdaArgument(
                Statement.Expression.Invoke(variableReference)
                    .AsExpression()),
            new ValueArgument(message ?? string.Empty)
        };

        return Statement.Expression.Invoke(
                "Assert",
                "Throws",
                arguments,
                new List<Type> { exception })
            .AsStatement();
    }

    private static ExpressionStatementSyntax Are(
        AssertType assertType, IArgument expected, IArgument actual,
        string? message)
    {
        if (expected == null)
        {
        ...
        return Statement.Expression.Invoke(
                "Assert",
                Enum.GetName(typeof(AssertType), assertType),
                arguments)
            .AsStatement();
    }
    ...
    private static ExpressionStatementSyntax Is(bool expected, IArgument actual, string? message)
    {
        ...
        return Statement.Expression.Invoke("Assert", expected ? "IsTrue" : "IsFalse", argument)
            .AsStatement();
    }
}
