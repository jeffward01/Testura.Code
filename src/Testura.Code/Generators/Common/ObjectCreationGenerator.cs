...
public static class ObjectCreationGenerator
{
    ...
    public static ExpressionSyntax Create(
        Type type, IEnumerable<Type> genericTypes = null,
        IEnumerable<ExpressionSyntax> initialization = null)
    {
    ...
    /// <returns>An object creation expression.</returns>
    public static ExpressionSyntax Create(
        Type type, IEnumerable<IArgument> arguments, IEnumerable<Type>? genericTypes = null,
        IEnumerable<ExpressionSyntax>? initialization = null)
    {
        ObjectCreationExpressionSyntax objectCreationExpressionSyntax;

        if (genericTypes != null && genericTypes.IsNotNullOrEmpty())
        {
            objectCreationExpressionSyntax = ObjectCreationExpression(
                GenericGenerator.Create(type.Name, genericTypes.ToArray()));
        }
        else
        {
        ...
        if ((arguments != null && arguments.Any()) || initialization == null || initialization.Any().IsFalse())
        {
            objectCreationExpressionSyntax =
                objectCreationExpressionSyntax.WithArgumentList(
        ...
        if (initialization != null && initialization.Any())
        {
            ...
            objectCreationExpressionSyntax = objectCreationExpressionSyntax.WithInitializer(
                InitializerExpression(
                    ...
                    SeparatedList<ExpressionSyntax>(syntaxNodeOrToken)));
        }
