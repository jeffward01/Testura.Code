...
/// <summary>
///     Provides the functionality to generate references and reference chains. Example of generated
///     code: "<c>test</c>", "<c>test.MyMethod()</c>".
/// </summary>
public static class ReferenceGenerator
{
    ...
    public static ExpressionSyntax Create(VariableReference reference)
    {
        ...
        if (reference is MethodReference methodReference)
        {
            ...
            {
                baseExpression = InvocationExpression(
                        GenericGenerator.Create(
                            methodReference.Name,
                            methodReference.GenericTypes.ToArray()))
                    .WithArgumentList(
                        ArgumentGenerator.Create(methodReference.Arguments.ToArray()));
            }
            else
            {
    ...
    private static ExpressionSyntax Generate(
        ExpressionSyntax expressionSyntax, MemberReference reference)
    {
        while (true)
        {
            switch (reference)
                        ...
                        expressionSyntax = InvocationExpression(
                                MemberAccessExpression(
                                    ...
                                    GenericGenerator.Create(
                                        ...
                                        memberReference.GenericTypes.ToArray())))
                            .WithArgumentList(
                                ArgumentGenerator.Create(memberReference.Arguments.ToArray()));
                    }
                    else
                    {
                        expressionSyntax = InvocationExpression(
                                MemberAccessExpression(
                                    SyntaxKind.SimpleMemberAccessExpression,
                                    expressionSyntax,
                                    IdentifierName(memberReference.Name)))
                            .WithArgumentList(
                                ArgumentGenerator.Create(memberReference.Arguments.ToArray()));
                    }
