...
public static class ArgumentGenerator
{
    ...
    public static ArgumentListSyntax Create(params IArgument[] arguments)
    {
        ...
        return SyntaxFactory.ArgumentList(
            SyntaxFactory.SeparatedList<ArgumentSyntax>(convertedArguments.ToArray()));
    }
    ...
    internal static List<SyntaxNodeOrToken> ConvertArgumentsToSyntaxNodesOrTokens(
        params IArgument[] arguments)
    {
        ...
        foreach (var argument in arguments)
        {
            list.Add(
                argument != null
                    ? argument.GetArgumentSyntax()
                    : new ValueArgument("null").GetArgumentSyntax());
