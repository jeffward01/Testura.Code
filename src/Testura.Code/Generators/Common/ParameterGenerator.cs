...
public static class ParameterGenerator
{
    ...
    internal static ParameterSyntax Create(Parameter parameter)
    {
        ...
        switch (parameter.Modifier)
        {
            case ParameterModifiers.Out:
                parameterSyntax =
                    parameterSyntax.WithModifiers(TokenList(Token(SyntaxKind.OutKeyword)));

                break;
            case ParameterModifiers.Ref:
                parameterSyntax =
                    parameterSyntax.WithModifiers(TokenList(Token(SyntaxKind.RefKeyword)));

                break;
            case ParameterModifiers.This:
                parameterSyntax =
                    parameterSyntax.WithModifiers(TokenList(Token(SyntaxKind.ThisKeyword)));
    ...
    internal static ParameterListSyntax ConvertParameterSyntaxToList(
